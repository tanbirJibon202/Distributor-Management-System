...
async function main() {
  ...
  const superAdminPassword = await bcrypt.hash('SuperAdmin123!', BCRYPT_ROUNDS);
  ...
  const dhakaManager = await prisma.user.upsert({
  ...
  const ctgManager = await prisma.user.upsert({
  ...
  if (!existingOrder) {
    ...
    await prisma.order.create({
      data: {
        ...
        items: { create: [{ productId: product.id, quantity, unitPrice, subTotal }] },
      },
    });
  ...
  console.log(`Super admin login: ${superAdmin.email} / SuperAdmin123!`);
  console.log(`Branch manager login: ${dhakaManager.email} / Manager123!`);
  console.log(`Field SR login: ${srs[0].email} / FieldSr123!`);
}
